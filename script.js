...
const firebaseConfig = {
    apiKey: "AIzaSyB5UMaVQkBmTKfv-opwFj-3sSWkjEH8_So",
    authDomain: "student-finance7.firebaseapp.com",
    projectId: "student-finance7",
    storageBucket: "student-finance7.firebasestorage.app",
    messagingSenderId: "705894538761",
    appId: "1:705894538761:web:a3dac21c9c57e6643ba2d8",
    measurementId: "G-XM06W7G2GK"
};
...
function saveExpense(e) {
    ...
    saveExpenseToFirestore(expense, isNew);
    closeExpenseModal();
    refreshAll();
}
...
function saveBudget(e) {
    ...
    if (editId) {
        ...
        if (existing) {
            ...
            isNew = false;
            showToast('Budget updated for this category!', 'success');
        } else {
            state.budgets.push(budget);
            showToast('Budget set!', 'success');
        }
    }
    saveBudgetToFirestore(budget, isNew);
    closeBudgetModal();
    refreshAll();
}
...
function saveSavingsGoal(e) {
    ...
    saveSavingsGoalToFirestore(goal, isNew);
    closeSavingsModal();
    refreshAll();
}
...
function addToSavings(e) {
    ...
    if (goal) {
        ...
        if (goal.current >= goal.target) {
            ...
            showToast('Added ' + fmt(amount) + ' to ' + goal.name, 'success');
        }
        savingsGoalsCollectionRef().doc(goal.id).update({ current: goal.current }).catch(function (err) {
...
function saveDebt(e) {
    ...
    saveDebtToFirestore(debt, isNew);
    closeDebtModal();
    refreshAll();
}
...
function saveGroupExpense(e) {
    ...
    if (editId) {
        var idx = group.expenses.findIndex(function (ex) { return ex.id === editId; });
...
function deleteGroupExpense(groupId, expenseId) {
    var group = state.groups.find(function (g) { return g.id === groupId; });
    if (!group) return;
    group.expenses = group.expenses.filter(function (ex) { return ex.id !== expenseId; });
    saveGroupToFirestore(group);
    showToast('Expense deleted', 'warning');
    updateGroupPage();
}
